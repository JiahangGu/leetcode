#!/usr/bin/env python
# encoding: utf-8
# @Time:2020/9/27 15:23
# @Author:JiahangGu
class Solution:
    def longestPalindrome(self, s: str) -> str:
        """
        求回文子串的方式通常是从中心向两边扩展，直到不等的情况则可以得到回文字串。需要注意的是中心可能是一个字符，或者是None（长度偶数）
        遍历字符串求最长串即可。时间复杂度为O(n^2)。
        利用上述特性，还有DP解法。设dp[i][j]表示[i,j]是否是回文字串，则dp[i][j] = dp[i+1][j-1] if s[i] == s[j] else False
        最终只需要找出所有True的dp状态中最长的字串即可。时间复杂度为O(n^2)，空间复杂度为O(n^2)。
        :param s:
        :return:
        """
        '''
